...
def _prepare_mat2(mat1, mat2, *, allow_tf32):
    # Large T-Head FP32 workloads benefit from materializing a K-contiguous
    # transposed RHS in strict FP32. The T-Head fast-FP32 lowering is already
    # layout-insensitive, so copying there only adds overhead. Restrict the
    # copy to inference, where it cannot alter autograd view semantics, and
    # only apply it when M amortizes the copy.
    mat2_k_contiguous = (
        mat2.dim() == 2 and mat2.stride(0) == 1 and mat2.stride(1) > 1
    )
    if (
        not torch.is_grad_enabled()
        and not allow_tf32
        and mat1.dim() == 2
        and mat1.dtype == torch.float32
...
def _addmm_impl(bias, mat1, mat2, out, beta, alpha):
    ...
    mat2 = _prepare_mat2(mat1, mat2, allow_tf32=allow_tf32)
...
def addmm(bias, mat1, mat2, *, beta=1, alpha=1):
    logger.debug(
        "GEMS_THEAD ADDMM, [shape info]: [-, %s, %s, %s](batch, M, N, K), "
...
def addmm_out(bias, mat1, mat2, *, beta=1, alpha=1, out=None):
    logger.debug(
        "GEMS_THEAD ADDMM_OUT, [shape info]: [-, %s, %s, %s](batch, M, N, K), "
        "[A column-major]: %s, [B column-major]: %s, [bias column-major]: %s",
        mat1.shape[0],
        mat2.shape[1],
        mat1.shape[1],
...
def addmm_dtype_out(bias, mat1, mat2, out_dtype, *, beta=1, alpha=1, out):
    ...
    if mat1.dtype != mat2.dtype:
        raise RuntimeError(
            f"mat1 and mat2 must have the same dtype, but got "
