...
def add_add_silu(addend, residual_a, residual_b):
    """Fuse GraphCast's ordered FP32 add/add/SiLU chain into one Triton kernel."""

    logger.debug("GEMS_ASCEND ADD_ADD_SILU")
    ...
    if n_elements == 0:
        return output
    block_size = 8192 if n_elements >= 65536 else 1024
